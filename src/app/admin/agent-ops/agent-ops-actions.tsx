...
export function PromoCreateForm() {
  ...
  return (
    <div className="mt-4 flex flex-wrap gap-2 border-t border-border pt-4">
      <input
        type="text"
        placeholder="CODE e.g. DCS-SUMMER-20"
        value={code}
        onChange={(e) => setCode(e.target.value.toUpperCase())}
        className="min-w-[160px] flex-1 rounded-xl border border-border px-3 py-2 text-sm font-mono uppercase"
      />
      <input
        type="number"
        step="0.01"
...
export function PromoToggle({ promoId, active }: { promoId: string; active: boolean }) {
  ...
  return (
    <Button size="sm" variant={active ? "secondary" : "default"} onClick={toggle} disabled={pending}>
